...
class Vars{
	...
	public function set($s,$c,$t = NULL){ //set a global variable
		...
		$r = $sql->query("SELECT id,type FROM vars WHERE name='%s'",Array($s),0);
		if(isset($r['id'])){ //check if we need to update or add a new
			$sql->query("UPDATE vars SET value='%s',type='%s' WHERE name='%s'",Array($c,$type,$s));
		}else{
			$sql->query("INSERT INTO vars (name,value,type) VALUES('%s','%s',%d)",Array($s,$c,(int)$type));
		}
		return true;
	}
	public function get($s){
		...
		$res = $sql->query("SELECT value,type FROM vars WHERE name='%s'",Array($s),0);
	...
	public function delete($s){
		global $sql;
		$sql->query("DELETE FROM vars WHERE name='%s'",Array($s));
	}
}
$vars = new Vars();
?>